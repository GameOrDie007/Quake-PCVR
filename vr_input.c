/*
	...
	Team Beef's QuakeQuestSrc/OpenXrInput.c, brought across for PC.

	Their file has no JNI, EGL or Android reference in it at all - it is the
	OpenXR action set, the suggested bindings, controller state and haptics,
	and none of that is platform specific. Only the include line and the two
	logging macros change; every action, every binding path and every value
	below is theirs, unmodified.
...
void CreateAction(
    ...
    strlcpy(aci.actionName, actionName, sizeof(aci.actionName));
    strlcpy(aci.localizedActionName, localizedName ? localizedName : actionName, sizeof(aci.localizedActionName));
...
void TBXR_InitActions( void )
{
    // Create an action set.
    {
        ...
        strlcpy(actionSetInfo.actionSetName, "gameplay", sizeof(actionSetInfo.actionSetName));
        strlcpy(actionSetInfo.localizedActionSetName, "Gameplay", sizeof(actionSetInfo.localizedActionSetName));
